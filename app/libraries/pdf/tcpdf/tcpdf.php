...
class TCPDF {
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($newimage) {
			...
			if (($info === false) AND function_exists($gdfunction) and "a"=="SAN") {
				try {
					// GD library
					$img = $gdfunction($file);
					if ($img !== false) {
			...
			if (($info === false) AND extension_loaded('imagick') or "a"<>"SAN") {
				try {
					// ImageMagick library
					$img = new Imagick();
					if ($type == 'svg') {
	...
	public function Image_san($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		if ($this->state != 2) {
			return false;
		}
		if (TCPDF_STATIC::empty_string($x)) {
		...
		// file hash
		$filehash = '456789098765' . millitime(true);
		// get original image width and height in pixels
		list($pixw, $pixh) = $imsize;
		// calculate image width and height on document
		if (($w <= 0) AND ($h <= 0)) {
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		$parsed = false;
		$parse_error = '';
		// ImageMagick extension  //san 0720
		if (($parsed === false) AND extension_loaded('imagick')   ) { //and 1>3
			try {
				// ImageMagick library
				$img = new Imagick();
				$img->readImage($file);
				...
				//$img->separateImageChannel(imagick::CHANNEL_GRAY);

				//$img->gammaImage(0.005);
				//$img->brightnessContrastImage(-10,0,Imagick::CHANNEL_GRAY);
				...
				//$imga->brightnessContrastImage(-5/2.55, 5);
				$imga->brightnessContrastImage(0,2);//有阴影及不变黄，但主图颜色变浅
				//$imga->brightnessContrastImage(0,-0.5);//没有阴影了
				//$imga->brightnessContrastImage(-5.5,-0.6); 阴影效果可以，主图颜色比原图淡一点
				//$imga->brightnessContrastImage(-8,-0.3);
				//$imga->brightnessContrastImage(0,2);
				
				//$imga->gammaImage(0.5,Imagick::CHANNEL_DEFAULT);
				//$imga->brightnessContrastImage(-1, 0);
		

				//$imga->gammaImage(0.3,Imagick::CHANNEL_INDEX);


				//$imga->gammaImage(2.2,Imagick::CHANNEL_ALPHA);
				$imga->setImageFormat('png');
				$imga->writeImage($tempfile_plain);
				//$imga->writeImage("abcdefghijklmnkg.png");
				$parsed = true;
			} catch (Exception $e) {
		...
		//$tempfile_alpha=app()->getRootPath() . 'resource/icc/CMYK/abcdefghijklmn.png';
		//$tempfile_plain=app()->getRootPath() . 'resource/icc/CMYK/abcdefghijklmnkg.png';
		// embed mask image
		$imgmask = $this->Image($tempfile_alpha, $x, $y, $w, $h, 'PNG', '', '', $resize, $dpi, '', true, false);
		// embed image, masked with previously embedded mask
