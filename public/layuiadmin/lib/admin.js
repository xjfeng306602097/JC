...
/*!
 * 界面核心模块   
 * 2021-11-17 siliang 添加在移除/关闭子窗口时触发onbeforeunload和onunload方法
 * 2021-12-21 siliang 添加openTabsPage用于直接打开标签页
 */
...
layui.define('view', function(exports){
    ...
    ,admin = {
        ...
        ,tabsBodyChange: function(index, options){
            ...
            if (self == top) {
                if (options.url !== activeRoute) {
                    activeRoute = options.url;
                    if (window.frames[index] && window.frames[index].onshow) {
                        window.frames[index].onshow();
                    }
                }
            }
        ...
        //打开指定的标签页
        ,openTabsPage: function(href, title) {
    ...
    var events = admin.events = {
        ...
        ,note: function(othis){
            ...
            events.note.index = admin.popup({
                ...
                ,success: function(layero, index){
                    ...
                    ,value = note === undefined ? 'The contents of the notes are stored locally, so that even if you close the browser, the next time you open it, the last record will still be read. Its a very small and useful local memo' : note;
        ...
        ,wiki: function(){
            admin.popupRight({
                id: 'LAY_adminPopupWiki'
                ,area: '380px'
                ,success: function(){
                    view(this.id).render('system/wiki');
                }
            });
        }
        ...
        ,closeOtherTabs: function(type){
            ...
            if(type === 'all'){
                events.triggerUnload($(APP_BODY).find('.'+ TABS_BODY+ ':gt(0)'));
                ...
                events.triggerUnload($('.'+ TABS_REMOVE));
            }
        }
        ...
        //手动触发unload事件
        ,triggerUnload: function(ele){
            for(var x in ele) {
                if (parseFloat(x).toString() != "NaN") {
                    var iframe = $(ele[x]).find('iframe')[0];
                    if (iframe !== undefined) {
            ...
            ele.removeClass('layui-show').removeClass('layadmin-tabsbody-item');
            ele.hide();
            // 30秒后移除dom
            setTimeout(function() {
                ele.remove();
    ...
    element.on('tabDelete('+ FILTER_TAB_TBAS +')', function(obj){
        ...
        if (obj.index) {
            events.triggerUnload(admin.tabsBody(obj.index));
        }
        setThisRouter(othis);
