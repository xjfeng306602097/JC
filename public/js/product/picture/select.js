/**
 ...
 @Name：makro 
 @Author：makro
 @Site：http://ho.makrogo.com/makroDigital/product/pictureSelect
...
layui.config({
    ...
    var itemCode = getUrlRelativePath(4);
    var storage = layui.data(setter.tableName);

    var isTemp = getUrlSearchParams('tempUpload') == 1;// 是否为临时上传图片
    var isEdit = false;

    // 当前列表数据
    var listData = {};

    $('.title span').text('(' + itemCode + ')');
    $('input[name="searchKey"]').val(itemCode);
    var current_page = 1,
        current_limit = 12,
        current_itemCode = itemCode,
        current_temp = isTemp ? undefined : 0;
    function loadPictureData(success) {
        ...
        $.ajax({
            ...
            type: getApiMethod('product.picture.page'),
            headers: {
                ...
                if (result.code === "0000") {
                    ...
                    if (list && list.length > 0) {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            var item = list[i];
                            listData[item.basicInfo.id] = item;
                            ...
                            } else if (!permission.verify('product:picture:delete')) {
    ...
    $(document).on('click', '.choosePicture', function() {
        ...
        var json = JSON.stringify(listData[picID]);
        if (isEdit) {
    ...
    $(document).on('click', '.picture-action button', function() {
        ...
        if (action == 'alert') {
            ...
            parent.layer.confirm('Confirm for delete?', {
                ...
                $.ajax({
                    url: getApiUrl('product.picture.delete', {ids: picID}),
