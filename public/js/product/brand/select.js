/**
 ...
 @Name：makro 
 @Author：makro
 @Site：http://ho.makrogo.com/makroDigital/productBrand/pictureSelect
...
layui.config({
    ...
    var brandID = getUrlRelativePath(4);
    var storage = layui.data(setter.tableName);

    var isEdit = false;

    if (window.brandName !== undefined) {
        $('.title span').text('(' + window.brandName + ')');
    }
    $('input[name="searchKey"]').val(brandID);
    var current_page = 1,
        current_limit = 12,
        current_brandid = brandID;
    function loadPictureData(success) {
        ...
        $.ajax({
            url: getApiUrl('product.brand.picture.page'),
            type: getApiMethod('product.brand.picture.page'),
            headers: {
                ...
                if (result.code === "0000") {
                    ...
                    if (list && list.length > 0) {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            } else if (!permission.verify('product:brand:picture:delete')) {
    ...
    $(document).on('click', '.picture-action button', function() {
        ...
        if (action == 'alert') {
            ...
            parent.layer.confirm('Confirm for delete?', {
                ...
                $.ajax({
                    url: getApiUrl('product.brand.picture.update', {id: picID}),
                    type: getApiMethod('product.brand.picture.update'),
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": 'Bearer ' + storage.access_token
                    },
