...
layui.config({
    ...
    function getMessageTemplateList(){
        table.render({
            ...
            ,toolbar: true
            ,toolbar: '#messageTemplateToolbar'
            ...
            ,cols: [[
                ...
                ,{field:'title', width: 300, title: 'Title', fixed: 'left', sort: true, fixed: 'left' }
                ,{field:'status', width: 90, title: 'Status', templet: '#content-messageTemplate-list-status'}
    ...
    // 搜索
    form.on('submit(LAY-messageTemplate-front-search)', function(obj) {
    ...
    // 重置搜索
    form.on('submit(LAY-messageTemplate-front-reset)', function(obj) {
        form.val('messageTemplateSearch', {
            title: '',
            status: '',
        });
        form.render();
    ...
    // 监听头部工具栏
    table.on('toolbar(content-messageTemplate-list)', function(obj){
        ...
        switch (obj.event) {
            ...
            case 'add':
                var index_page = layer.open({
                    ...
                    ,content: '/makroDigital/messageTemplate/add'
                    ,maxmin: true
                    ,area: ['600px', '455px']
                    ,btn: ['Save', 'Cancel']
                    ,yes: function (index, layero) {
                        var iframeWindow = window['layui-layer-iframe' + index],
                            submitID = 'LAY-messageTemplate-add-submit',
                        ...
                        iframeWindow.layui.form.on('submit(' + submitID + ')', function(obj) {
                            ...
                            $.ajax({
                                url: getApiUrl('message.template.add'),
                                type: getApiMethod('message.template.add'),
                                ...
                                success: function(result) {
                                    if (result.code === "0000") {
                                        ...
                                        table.reload('messageTemplateTable');
                                    } else {
                                        layer.msg(result.msg);
                                    }
                                }
            ...
            case 'edit':
                if (data.length==0) {
                    ...
                    var index_page = layer.open({
                        ...
                        ,id: 'editMessageTemplate'
                        ,content: '/makroDigital/messageTemplate/edit/' + id
                        ,maxmin: true
                        ,area: ['600px', '455px']
                        ,btn: ['Save', 'Cancel']
                        ,yes: function (index, layero) {
                            var iframeWindow = window['layui-layer-iframe' + index],
                                submitID = 'LAY-messageTemplate-edit-submit',
                                submit = layero.find('iframe').contents().find('#' + submitID);
                            ...
                            iframeWindow.layui.form.on('submit(' + submitID + ')', function(obj) {
                                ...
                                $.ajax({
                                    url: getApiUrl('message.template.update', {id: id}),
                                    type: getApiMethod('message.template.update'),
                                    data: JSON.stringify(mydata),
                                    headers: {
                                        "Content-Type": "application/json",
                                        "Authorization": 'Bearer ' + storage.access_token
            ...
            case 'delete':
                if (data.length==0) {
                    ...
                    layer.confirm('Confirm for delete?', {
                        ...
                        var id = data[0].id;
                        $.ajax({
                            url: getApiUrl('message.template.delete', {ids: id}),
                            type: getApiMethod('message.template.delete'),
                            headers: {
                                "Content-Type": "application/json",
                                "Authorization": 'Bearer ' + storage.access_token
                            },
