...
layui.config({
    ...
    function pagesReview(){
        ...
        for(var i=0;i<pageMap.length;i++){
            ...
            _Html=_Html + '         <img src="' + pageMap[i].canvasCode.previewUrl +'"  data="'+pageMap[i].pageNo+'"  >';
            _Html=_Html + '     </div>';
    ...
    $(document).on("click",".choosePage",function(){
        ...
        if (pageSort){
            ...
            _JC.templateAffair().insertPage(null, function(newPageSort) {

                var lastIndex=_JC.templateData.templatePages.length-1;
                var pageCode=_JC.templateData.templatePages[lastIndex].pageCode;
                var dupNo=_JC.pagesDuplicate[lastIndex][0].No;

                //获取该页面
                var canvasCode=pageMap[pageSort*1].canvasCode;
                    canvasCode.pageCode=pageCode;
                    canvasCode.No=dupNo;
                    canvasCode.isValid=0;

                if (!canvasCode.hasOwnProperty("previewUrl")){
                    canvasCode.previewUrl="";
                }

                _JC.pagesDuplicate[lastIndex][0]=canvasCode;
                console.log("pageCode= " + _JC.pagesDuplicate[lastIndex][0].pageCode);
                console.log("No= " + _JC.pagesDuplicate[lastIndex][0].No);
                console.log(_JC.pagesDuplicate[lastIndex][0].objects);
                parent.renderPagesItem();

                /*var pageParm = {};
                pageParm.pageCode = pageCode;
                pageParm.file = "";

                var prePageZoom=parent.canvas.getZoom();
                _JC.templateAffair().cutoverTemplatePage(pageParm, async function() {
                    
                    parent.zoomScale(false,prePageZoom * 100);
                    console.log("importPage canvas");
