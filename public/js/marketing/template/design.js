 ...
 */
//引入设计页模块JS
var canvas, context, _JC, _CMYK, _DB, userID, slider, mmDetailsData,designFont,lockStatus,designerConfig,myColorData,myTextStyle,allFonts,loadObject,timer,timerProcess,timeTask,timing;
...
layui.config({
    ...
    requireJS(["rotate","pageLayer", "config", "designFunction", "canvas", "JC", "pagesEvent", "cmykColor"], function() {    
        ...
        //加载loading层
        $(".loadingTip").remove();
        var loadEvent = layer.load(2, {time: 100000,shade: [0.3, '#393D49']});
                        ...
                        for(var i=0;i<_templatePageList.length;i++){
                            ...
                            if (_templatePageList[i]!=undefined){
                                ...
                                if (_templatePageList[i].isValid * 1 ===1){
                                    ...
                                    if (_templatePageList[i].content.duplicate[0].hasOwnProperty("version")){
                        ...
                        for(var i=0;i<templatePageList.length;i++){
                            ...
                            if (templatePageList[i]!=undefined){
                                ...
                                if (templatePageList[i].isValid * 1 ==1){
                                    ...
                                    //组装模板单页信息集
                                    var tmp={};
                                    tmp.sort=templatePageList[i].sort*1;
    ...
    window.onbeforeunload = function() {
        console.log("解锁MM current_id"+current_id);
        http.proxyRequest({
            url: getApiUrl('marketing.template.unlock', {code: current_id}),
