/**
 ...
 @Name：makro 
 @Author：makro
 @Site：http://ho.makrogo.com/makroDigital/marketingColor/edit
...
layui.config({
    ...
    // 获取颜色信息
    $.ajax({
        url: getApiUrl('marketing.color.detail', {id: current_id}),
        type: getApiMethod('marketing.color.detail'),
        headers: {
        ...
        success: function(result) {
            if (result.code === "0000") {
                form.val('colorEdit', {
                    "rgb": result.data.rgb,
                    "hex": result.data.hex,
                    "cmyk": result.data.cmyk,
                    "sort": result.data.sort,
                });
                setColor(result.data.rgb);
            } else {
